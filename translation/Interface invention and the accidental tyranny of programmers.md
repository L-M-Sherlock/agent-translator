...
更糟糕的是，即便具备了这两点，通常也还是不够。许多界面的创新[也需要深厚的领域洞察力](https://numinous.productions/ttft/#how-to-invent-hindu-arabic-numerals)。试想一下，Steinberg 是在积累了多年音乐家和制作人经验之后，才发明了（Cubase 中的）时间线编辑器；而 Bricklin 是在哈佛商学院就读期间，才发明了动态电子表格（VisiCalc）。
...
我怀疑我们之所以陷入这种境地，是因为这些技能之间存在着严重的不对称性。没有设计能力或领域洞察力的编程，依然**能够**产出可运行的界面——尽管它们往往很无聊或存在缺陷——但如果没有编程能力，再好的设计或领域洞察力也只能停留在画板上。因此，至少对于独立发明者来说，几乎所有能运行的界面都是由程序员创建的……而这些程序员是否具备其他技能就不好说了。如果新界面的瓶颈主要在于富有想象力的设计工作和深厚的领域洞察力，那么这种技能上的不对称性，实际上把筛选压力施加在了错误的技能上。
...
[一个流行的 AI 基准测试提出了这样一个问题](https://metr.org/blog/2025-03-19-measuring-ai-ability-to-complete-long-tasks/)：我能把一项工程任务交给一个无人监督的 AI 多久？对于原型制作，我一直在观察一个关键的相关指标：在代码库彻底崩溃、损坏且无法维护之前，人类（在没有技术监督的情况下）可以利用智能体迭代多久？在过去的一年里，我看到这个数字从几个小时变成了几个星期。这种进步很大程度上归功于模型变得越来越聪明，但我的感觉是，其中一部分也源于经验和策略的积累。微小的流程改进就能对原型的生命周期产生巨大影响。（有关一些战术层面的建议，请参阅下文的脚注。）
...
- **为粗糙的原型编写严谨的代码（Rigorous code for sloppy prototypes）**。当我自己手写原型代码时，我通常会避开单元测试、类型检查、状态机、编写文档和正式的数据建模这些「严谨」的实践——它们往往会拖慢进度。但使用智能体时，即便是一个粗糙的原型，采用这些严谨的实践也是大有裨益的：它们能提供额外的上下文和安全护栏。而且智能体会自动处理掉大部分通常需要人工维护的开销。
- **少量的编程指导（Fractional programming supervision）**。哪怕只是一位经验丰富的程序员提供一点点微小的支持，也能让这个用「胶带」粘起来的系统维持运转得久得多。而且我认为，在这个新世界里，不懂编程的人更容易开口去寻求这类支持，因为他们的想法已经部分地实体化为一个很酷的原型了——只不过可能还有点 Bug 而已。
- **速率分层（Pace layers）**。当我们在同一个原型上协同工作时，我们通过将系统划分为不同的架构层，并将那些未经审查的智能体代码限制在最外层，从而更长久地推迟了系统的崩溃。对于同步系统或敏感的领域逻辑等核心组件的修改，则必须经过严格的代码审查。
- **自我监督（Self-supervision）**。如今的编程智能体经常会做出糟糕的技术决策并留下烂摊子。但是，在一定程度上，不懂编程的人可以直接要求其他智能体去清理这些乱象并简化代码。如果模型能够驱动自身的反馈循环（例如，使用测试用例或浏览器自动化工具），这一切就会运作得更好。
- **只读式编程（Read-only programming）**。我不会说西班牙语，但我通常能凭借自己那点微薄的意大利语和拉丁语知识作为参照，大致看懂书面的西班牙语。同样，不懂编程的人如果仔细检查，通常也能发现智能体代码中的问题。「等等，你为什么要重复写这段逻辑？这里应该调用我们已经写好的那个接口啊……」
- **接受崩溃（Accept collapse）**。既然现在编写代码的成本已经变得极其低廉，那么代码库时不时地自我毁灭也没什么大不了的，只要你能从中汲取足够的经验教训，用来指导你设计下一个原型就足够了。
