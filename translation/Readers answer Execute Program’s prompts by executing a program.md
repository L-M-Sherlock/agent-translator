...
这看起来像 Quizlet 风格的抽认卡：你必须把正确答案敲进去；但它并不是。它是一个真正的解释器，会真的对你的程序求值。因此你可以写 `[{name: “Boromir”}, {name: “Aragorn”}]`，也可以写 `[“Boromir”, “Aragorn”].map(n => ({name: n})`——两者都正确。我喜欢这种设计模型，因为它让课程更接近真实语境：[Enabling environments focus on doing what’s enabled](https://notes.andymatuschak.org/z2etsLyP1LJUwNDPCwvRdUG)。
...
一个相关、但更具野心的构想：[What might it mean to situate games like Shenzhen I/O inside pro environments like an IDE?](https://notes.andymatuschak.org/zFS2bUsK1vzv1M7cYcMFdCV)

也相关：[Executable books](https://notes.andymatuschak.org/zLNcy2JiH3AGNaZjYkHzK9P)
