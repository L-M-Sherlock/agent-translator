...
这看起来像 Quizlet 风格的抽认卡：你必须把正确答案敲进去；但它并不是。它是一个真正的解释器，会真的对你的程序求值。因此你可以写 `[{name: “Boromir”}, {name: “Aragorn”}]`，也可以写 `[“Boromir”, “Aragorn”].map(n => ({name: n})`——两者都正确。我喜欢这种设计模型，因为它让课程更接近真实语境：[赋能环境会聚焦于所赋能之事](https://notes.andymatuschak.org/z2etsLyP1LJUwNDPCwvRdUG)。
