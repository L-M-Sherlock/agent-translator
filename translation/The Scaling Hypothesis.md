...
而元学习也是同理：当模型看过足够多只在每个样本内略有变化的算法实例后，把每个任务分开学就会变得困难，于是它学到的其实**就是**更通用的算法；这会产生损失更低的子模型，而那些竞争子模型要么预测得不好，要么膨胀得无法接受。
...
DeepMind^[这里主要指 Demis Hassabis；至于 Shane Legg 的看法，我并不确定，不过考虑到他在创办 DeepMind 时做出的判断与后续评论，他大概并没有明显改变自己“AI 将由已经实现的指数级算力增长所赋能”的看法，也没有明显改变他对约 `2028` 年实现 AGI 的预测；这也与 Metaculus 的最新预测大体一致。] 所持有的，可以说是一种“弱缩放假说”（相关链接：[关于 Demis Hassabis 的维基条目](https://en.wikipedia.org/wiki/Demis_Hassabis)、[关于 Shane Legg 的维基条目](https://en.wikipedia.org/wiki/Shane_Legg)、[Shane Legg 的当前看法](https://www.vetta.org/2009/12/tick-tock-tick-tock-bing/)、[Shane Legg 在 2009 年的预测](https://www.vetta.org/2009/12/the-teenies/)、[Shane Legg 在 2018 年的评论](https://web.archive.org/web/20210426084422/https://www.stuff.co.nz/technology/103500435/google-deepmind-founder-and-leader-in-artificial-intelligence-returns-to-hamilton)、[Shane Legg 关于约 `2028` 年实现 AGI 的预测](https://www.vetta.org/2010/12/goodbye-2010/)、[Metaculus 的相关问题](https://www.metaculus.com/questions/questions/3479/when-will-the-first-artificial-general-intelligence-system-be-devised-tested-and-publicly-known-of/)、[Metaculus 的相关预测](https://www.metaculus.com/questions/questions/1394/will-ai-progress-surprise-us/)）：他们认为，AGI 需要我们“找到正确的算法”，相当于逐模块复制哺乳动物大脑；而这些模块虽然按当代标准看会极其庞大、极其昂贵（这也正是算力重要的原因——它能给我们“一种更强有力的工具，去寻找正确的算法”），但它们仍然需要被一块一块地发明出来、微调出来，在最终组装完成之前，几乎不会有太多风险或惊喜。
